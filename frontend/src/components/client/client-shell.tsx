...
import { ClientSidebarNav } from "./client-sidebar";
import { ClientTopbar, type ClientUser } from "./client-topbar";
...
/**
 * Client shell: collapsible desktop sidebar rail with hover-to-expand (a temporary
 * overlay) and a pin toggle (persists collapsed/expanded), mobile drawer,
 * topbar and the PWA install prompt.
 */
export function ClientShell({
  ...
  useEffect(() => {
    try {
      setPinnedCollapsed(localStorage.getItem("litch:client-sidebar-collapsed") === "1");
    } catch {}
  }, []);
  ...
  function togglePin() {
    setPinnedCollapsed((c) => {
      const next = !c;
      try {
        localStorage.setItem("litch:client-sidebar-collapsed", next ? "1" : "0");
      } catch {}
      return next;
    });
  }
    ...
    <TourProvider audience="client">
      <div className={cn("min-h-screen bg-cloud lg:grid", pinnedCollapsed ? "lg:grid-cols-[4rem_1fr]" : "lg:grid-cols-[16rem_1fr]")}>
        ...
        >
          ...
          >
            <ClientSidebarNav collapsed={collapsed} pinnedCollapsed={pinnedCollapsed} onTogglePin={togglePin} />
          </div>
        </aside>
        ...
        <div className="flex min-h-screen min-w-0 flex-col">
          <ClientTopbar onMenuClick={() => setMobileOpen(true)} user={user} notifications={notifications} />
